...
class RelativePositionalEmbedding(tf.keras.layers.Layer):
    ...
    def relative_logits(self, inputs):
        ...
        inputs = tf.reshape(inputs, [-1, self.input_height, self.input_width, self.key_dim])  # Merge bs and heads, for supporting TFLite conversion
        ...
        return tf.reshape(logits, [-1, self.num_heads, self.input_height, self.input_width, self.position_height, self.position_width])
