...
from .down_sampling2d import SP
...
class SPPELAN(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.sp1 = SP(
            pool_size = self.pool_size , strides =self.strides, name ='sp1'
        )
        self.sp2 = SP(
            pool_size = self.pool_size , strides =self.strides, name ='sp2'
        )
        self.sp3 = SP(
            pool_size = self.pool_size , strides =self.strides, name ='sp3'
        )     
    ...
    def call(self, x):
        ...
        y = [self.conv1(x)]
        y.extend(sp(y[-1]) for sp in [self.sp1, self.sp2, self.sp3])
        return self.conv2( self.concat(y) ) 