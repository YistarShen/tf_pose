...
class WeightedFeatureFusion(tf.keras.layers.Layer):
    ...
    def __init__(self,  
                activation = 'relu',    
                epsilon : float= 1e-5,
                name : str='WeightFuse', **kwargs):
        super(WeightedFeatureFusion, self).__init__(name=name)

        if (not isinstance(epsilon,(float))):
            raise TypeError(f"bn_eps and  bn_momentum must be 'float' type @{self.__class__.__name__}"
                            f"but got eps:{type(epsilon)}"
            )
    ...
    def build(self, input_shape):
        if not isinstance(input_shape,list):
            raise TypeError('input must be List[Tensor]')
        
        self.num_features = len(input_shape)
        assert self.num_features >= 2,'len of input List[Tensor] must be >=2'
    ...
    def call(self, inputs):
        ...
        fusion_w = self.act(self.fusion_weights) 
        sum_features = []
