...
from lib.models.data_preprocessors.img_preprocessor import ImgPreProcess_Layer
...
def FastViT(
    ...
    nn = ImgPreProcess_Layer(name='ImgPreProcess_normalize')(inputs)
    stem_width = stem_width if stem_width > 0 else out_channels[0]
    nn = rep_conv_block(nn, stem_width, kernel_size=3, strides=2, deploy=deploy, name="stem_1_")
...
def fun_test(input_shape=(256,256), filters=128, kernel_size = 3):
...
@MODELS.register_module()
def FastViT_SA12_Test(input_shape=(256, 256, 3), num_classes=1000, deploy=False, activation="gelu", classifier_activation="softmax", pretrained="distill", **kwargs):
    num_blocks = [2, 2, 4, 4]
    out_channels = [64, 128, 256, 512]
    mlp_ratio = 4
    block_types = ["conv", "conv", "conv", "transformer"]
